...
export class DocConfigMock implements DocConfig {
  public async getIndex(): Promise<Index> {
    const index: Index = [
      [{ reference: 'key1', source_type: 'asciidoc', source: 'c:\\temp' }],
      [
        {
          reference: 'key1',
          document: 'test-data/brownfox.md',
        },
      ],
    ];
    return index;
...
export class ConfluenceServiceImplMock implements ConfluenceService {
  ...
  public getContentbyCredentials(
    ...
    const URI_CAPGEMINI =
      'https://adcenter.pl.s2-eu.capgemini.com/confluence/rest/api/content?spaceKey=JQ&title=Jump+the+queue+Home&expand=body.view';
    const URI_LOCAL =
      'http://localhost:8090/rest/api/content?spaceKey=CP&title=Jump+the+queue+Home+(Edited+for+Demo)&expand=body.view';

    const goodContent_path =
      'test-data/input/confluence/good/JumpTheQueueHome_capgemini.json';
    const multiplesPagesContent_path =
      'test-data/input/confluence/bad/multiplePages.json';
    const badFormatContent_path =
      'test-data/input/confluence/bad/badFormat.json';
