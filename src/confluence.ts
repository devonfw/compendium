    ...
    */
           ...
           if (node.children) {
             ...
             if (filter !== [] && filter !== null && filter !== undefined) {
               ...
               if (node.name === 'p') {
                 ...
                 const img: InlineImage = { kind: 'inlineimage', img: node.attribs.src, title: node.attribs.alt };
                 result.push(img);
         ...
         public linkContent(node: Array<any>): Paragraph | InlineImage {
           ...
           if (node.length === 1 && node[0].name === 'img') {
             const img: InlineImage = { kind: 'inlineimage', img: node[0].attribs.src, title: node[0].attribs.alt };
             result = img;
           } else {
         ...
         public list(node: Array<any>): Array<RichText | List | Paragraph | Link> {
           ...
           for (const li of node) {
             if (li.name === 'li') {
               for (const child of li.children) if (child.type === 'text' && child.data !== '\n') {
