...
export class MarkdownFileTextOut implements TextOut {
  public done: boolean = false;
  public asciidoctor = require('asciidoctor.js')();
  private outputFile: string;
  private TurndownService = require('turndown');
  ...
  public async generate(data: Array<Transcript>): Promise<void> {
    ...
    if (data.length < 1) {
      ...
      //html to markdown
      let turndownService = new this.TurndownService();
      let markdown = turndownService.turndown(docWithStyle);
      //create the file
      try {
        await extrafs.writeFile(this.outputFile + '.md', markdown, 'utf8');
      } catch (err) {
        throw err;
      }
    }
